...
export class NuevacuentaPage implements OnInit {
  ...
  async submit() {
    ...
    const { nombre, apellidos, celular, email, password } = formValue;
    const run = formValue.run?.toString().replace(/\./g, '').replace('-', '').toUpperCase() ?? '';

    try {
      const resultado = await this.supabaseService.registrarVeterinario(
        run,
        nombre!,
        apellidos!,
        email!,
